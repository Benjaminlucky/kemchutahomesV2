...
export default function ClientForgotPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/clients/forgot-password`, {
        method: "POST",
  ...
  if (sent) {
    return (
      <div className="rounded-xl border border-customPurple-200 bg-customPurple-50 p-6 text-center">
        ...
        <Link href="/client/login" className="mt-5 inline-block text-sm font-medium text-customPurple-600 hover:underline">
          ← Back to login
        </Link>
      </div>
    );
  ...
  return (
    <div>
      <div className="mb-8 text-center">
        ...
        <p className="mt-2 text-sm text-gray-500">Enter the email linked to your client portal account.</p>
      </div>
