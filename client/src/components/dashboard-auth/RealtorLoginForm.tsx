...
export default function RealtorLoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/realtors/login`, {
        method: "POST",
  ...
  if (success) {
    return (
      <div className="text-center">
        ...
        <p className="text-sm text-gray-500">Login successful — redirecting to your dashboard…</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="mb-8 text-center">
        <h2 className="text-3xl font-bold text-gray-900 md:text-4xl">Realtor sign in</h2>
        <p className="mt-2 text-sm text-gray-500">Access your recruits, commissions, and referral tools.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <p className="text-center text-sm text-gray-500">
          Not a realtor yet?{" "}
          <Link href="/signup" className="font-semibold text-customPurple-600 hover:underline">
            Join the network
          </Link>
        </p>
      </form>
