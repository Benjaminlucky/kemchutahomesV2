...
import { BarChart3, CheckCircle, Clock, Wallet, Info, ChevronDown, ChevronUp, Building2 } from "lucide-react";
...
const HEADERS = ["Estate / Reference", "Level", "Commission", "Status", "Date", ""];
...
async function fetchEarnings(page: number, limit: number, status: string): Promise<MyCommissionsResponse> {
  ...
  return (
    <div className="rounded-2xl border border-customBlack-100 bg-white p-5">
      <div className="mb-3 flex items-start justify-between">
        ...
        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-customPurple-50">
      ...
      <tr onClick={() => setOpen((o) => !o)} className="cursor-pointer hover:bg-customPurple-50/30">
        <td className="px-6 py-3.5">
          <div className="flex items-center gap-2">
            <Building2 size={13} className="text-customBlack-300" />
            <div>
              <p className="text-sm font-bold text-customBlack-900">{commission.estateName || "—"}</p>
...
export default function EarningsPanel({ initial, pageSize }: { initial: MyCommissionsResponse | null; pageSize: number }) {
  ...
  const { data, isFetching, error } = useQuery({
    queryKey: ["earnings", page, status],
    queryFn: () => fetchEarnings(page, pageSize, status),
    initialData: page === 1 && !status ? (initial ?? undefined) : undefined,
  ...
  const totalEarned = totals.reduce((acc, t) => acc + t.totalNet, 0);

  const levelBreakdown = ([1, 2, 3, 4] as const)
    .map((level) => {
      const rows = commissions.filter((c) => c.level === level);
      return { level, count: rows.length, total: rows.reduce((acc, c) => acc + c.netAmount, 0) };
    })
    .filter((l) => l.count > 0);
  const maxLevelTotal = Math.max(...levelBreakdown.map((l) => l.total), 1);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-0.5 text-sm text-customBlack-400">Commission earned from your property sales</p>
      </div>
      ...
      <div className="flex items-start gap-3 rounded-2xl border border-customPurple-100 bg-customPurple-50/60 p-4">
        ...
        <p className="text-xs leading-relaxed text-customBlack-600">
          You earn <strong>10%</strong> on every direct sale you close (Level 1). You also earn override commissions when
          realtors you recruited make sales — <strong>5%</strong> (L2), <strong>3%</strong> (L3), <strong>2%</strong> (L4).
          WHT of <strong>5%</strong> is deducted before payout. Commissions are held for a <strong>90-day</strong> clawback
          window then auto-approved.
        </p>
      </div>
      ...
      {levelBreakdown.length > 0 && (
        <div className="rounded-2xl border border-customBlack-100 bg-white p-6">
          ...
          <div className="space-y-3">
            {levelBreakdown.map(({ level, count, total: levelTotal }) => (
              <div key={level}>
